...
weightmatrix=zeros(numberoffeatures,monitoringtime*numberofclusters);
for i=1:numberoffeatures
    for j=1:monitoringtime
        for k=1:numberofclusters-1
            for l=k+1:numberofclusters
                if (clustermatrix(i,(k-1)*monitoringtime+j))==(clustermatrix(i,(l-1)*monitoringtime+j))
                    weightmatrix(i,(k-1)*monitoringtime+j)=weightmatrix(i,(k-1)*monitoringtime+j)+1;
                    weightmatrix(i,(l-1)*monitoringtime+j)=weightmatrix(i,(l-1)*monitoringtime+j)+1;
                end
            end
        end
    end
end
kernelnumbers=zeros(1,numberofclusters);
kerneltrainmatrix=zeros(1,monitoringtime*numberoffeatures);
t=zeros(numberofclusters,monitoringtime*numberoffeatures);
for i=1:numberoffeatures
    for j=1:monitoringtime
        for k=1:numberofclusters
            if weightmatrix(i,(k-1)*monitoringtime+j)==0
                kernelnumbers(1,k)=kernelnumbers(1,k)+1;
                t(k,(i-1)*monitoringtime+j)=1;
            end
        end
    end
end
for k=1:numberofclusters
    t(k,:)=clusterslabel(1,k)-t(k,:);
    kerneltrainmatrix=max(kerneltrainmatrix,t(k,:));
end
    ...
    for k=1:numberofclusters
     ...
     for k1=1:numberoffeatures
         for k2=1:monitoringtime
            if (s1p(k1,k2)==clustermatrix(k1,(k-1)*monitoringtime+k2))&&(s1p(k1,k2)==1)
